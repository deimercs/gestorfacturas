        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <div v-if="showProviderResults && filteredProviders.length" 
                   class="absolute z-10 w-full mt-1 bg-white shadow-lg rounded-md border">
                <div v-for="provider in filteredProviders" 
                     :key="provider.id"
              ...
              <div v-if="selectedProvider" class="mt-2 p-2 bg-gray-50 rounded">
                ...
                <p>{{ selectedProvider.name }} - NIT: {{ selectedProvider.document_number }}</p>
              </div>
              <button 
                type="button"
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const calculateTotals = () => {
        ...
        form.value.iva = form.value.subtotal * 0.19;
      ...
      const searchProviders = async () => {
        ...
        try {
          const response = await axios.get(`http://localhost:3001/providers/search?query=${providerSearch.value}`);
