      ...
      <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <table class="min-w-full">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Número Doc</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cliente</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Detalles</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Subtotal</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">IVA</th>
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="order in orders" :key="order.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-4">
                ...
                {{ order.details_list || order.items[0].details }}
              ...
              <td class="px-4 py-4 flex justify-center space-x-3">
                <button @click="showOrderDetail(order)" class="text-blue-600 hover:text-blue-800">
                  <EyeIcon class="h-5 w-5" />
                </button>
                <button @click="showFiles(order)" class="text-blue-600 hover:text-blue-800">
                  <FileIcon class="h-5 w-5" />
                </button>
                <router-link :to="`/orders/edit/${order.id}`" class="text-green-600 hover:text-green-800">
...
export default {
  ...
  setup() {    
    ...
    const orderIva = computed(() => 
      selectedOrder.value?.items?.reduce((sum, item) => sum + Number(item.iva), 0) || 0
    );
    ...
    const updateStatus = async (order) => {
      try {
        const response = await axios.put(`http://localhost:3001/orders/${order.id}/status`, {
          status: order.status
        });
