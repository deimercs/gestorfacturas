```vue
<!-- src/components/ProviderModal.vue -->
<template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        const response = await axios.post('http://localhost:3001/providers', form.value);
        emit('provider-added', response.data);
      } catch (error) {
        console.error('Error al crear proveedor:', error);
        errors.value.general = 'Error al crear el proveedor';
      }
    };
